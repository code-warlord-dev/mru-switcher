...
against it, not duplicated. Everything below describes what the shipped plugin does — this pass aligns it with the implementation (incl. the M6-T4 socket-permissions follow-up, issue #59).
...
| 2026-09-20 | M6-T4 follow-up (issue #59): socket mode `0600` enforced by the plugin (`fchmod` on the bound listener fd); parent-directory `0700` reworded as operator responsibility; T-FUZZ-01 parse-path fuzz harness added and running under the `sanitize` CI job |
