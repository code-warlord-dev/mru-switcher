...
- [ ] Bindings first-class (ADR-022; docs+packaging, no plugin change): ADR in DECISIONS.md; `examples/mru-switcher-bindings.lua` (Tab-release Lua recipe); `scripts/setup-bindings.sh` (backend detect, --force+backup, conflict check via `hyprctl binds -j`, exit codes 0-6; sandbox-tested); CI installer job extended; README/USER/API/COMPAT/CHANGELOG updated; bisect memo `research/2026-09-20-lua-modifier-release-bisect.md`; SPEC §14.6/REQ-DIST-018 citation in script header to be verified before PR
