...
- [ ] ADR-028 border styles pulse / dim (REQ-UI-013/014) — **code+tests+docs DONE on `feat/adr-028-border-styles` (054af25), commits pending**: `pulse` (colour throb via `HyprlandPulseTimer` / `wl_event_loop_add_timer`, `pulse_period_ms` clamp [200,10000], fail-soft warn-once on schedule failure/throw), `dim` (per-window `opacity`/`opacity_inactive` alpha via new capture slots, restore-by-value, unreadable skip, `dim_alpha>=1` disables; `dimAround` rejected — no public plugin setter); `darker_color` helper; `BorderStyleParams` + `PulseTimerPort` pure port; V2 + sidecar-09/10 + cfg-09/10 + t_ui_013×6/t_ui_014×4 + t_ui_07 update; SPEC/REQ-TRACE/COMPAT/USER/API/examples/CHANGELOG aligned; ctest 19/19, plugin .so builds, clang-format clean. REMAINING: atomic commits + final report; nest smoke
