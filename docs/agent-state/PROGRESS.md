...
- [ ] Live-host (Lua backend) plugin-config dead end — diagnosed 2026-09-22, no plugin change: **report `reports/2026-09-22-lua-host-plugin-config-dead-end.md`**; every channel for `plugin:mru-switcher:*` is dead on the Lua host (hyprctl keyword disabled by build; `hl.config` + config file reject `plugin.*`; `hl.keyword` nil; `hl.get_config` nil for plugin keys; `hl.plugin.load(path, config)` exists but Lua-path load fails silently; Omarchy `o.*` has no config facility) ⇒ plugin runs on compiled defaults (`ui=null` ⇒ no visual feedback; M5 overlay unavailable); plugin delivered via root-owned `/var/cache/hyprpm/<user>/` (enabled=true ⇒ survives reboot); follow-ups human-gated: upstream research (Lua `plugin {}` support), COMPAT/USER honesty pass (possibly needs-adr), newer-Hyprland nest check
