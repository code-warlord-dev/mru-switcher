...
- **N3 closes the origin with `dispatch killactive`** (active window = origin during the session). The plugin pruned the snapshot and kept the session Active (two candidates left), which is what makes the REQ-R-002 leg meaningful: cancel with a dead origin ⇒ no focus move.
