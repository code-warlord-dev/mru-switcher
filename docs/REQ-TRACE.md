...
| REQ-UI-012 | T-UI-08 (impl test names `t_ui_012_*`, 5 cases: selection-visible, invalid-target skip, degraded skip, end-reason forwarded, navigator-throw-swallowed), `cfg_08_selection_follow_workspace_default`, `sidecar_08_selection_follow_workspace_overlay`, + sidecar_01 extension | ADR-026: `ui=border` keeps the selection visible — `selection_follow_workspace` (default `true`) activates the selection's workspace via adapter-private `HyprlandWorkspaceNavigator` (`CMonitor::changeWorkspace`, `noFocus`; no window/keyboard focus — REQ-UI-006/ADR-023 intact); monitors restored on cancel, left on apply; gate no-op navigator when `false`; fail-soft warn-once on elevation/restore errors (REQ-UI-001); effective next session (REQ-CFG-002). **Deviations:** (1) the adapter's restore-through-`changeWorkspace` path is not unit-testable without the compositor — `end(reason)` forwarding + fail-soft tested instead, restore path verified in the nest (cancel from an elevated workspace returns to session-start); (2) `begin()`/`end()` pair even on degraded sessions (begin is part of a clean slate, no elevation runs). COMPAT mechanism row updated; final nest smoke recorded 2026-09-22 (view follows selection, focus untouched, cancel restores) | M4 |
