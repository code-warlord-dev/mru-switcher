...
| 1.0.0 | `v1.0.0` | M6 | v0.56.2 (`efb5099`), single pin in `hyprpm.toml` `commit_pins` | Stable dispatcher names, config keys, snapshot/apply semantics; **contract freeze declared in M6-T1 (issue #50)**; plugin-side pin = the tagged commit itself (Variant A self-pin: PR merged → pin set to main HEAD → tag placed on it); **tag pending the explicit human release command** |
