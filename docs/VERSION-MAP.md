...
| 1.0.0 | `v1.0.0` | M6 | v0.56.2 (`efb5099`), single pin in `hyprpm.toml` `commit_pins` | Stable dispatcher names, config keys, snapshot/apply semantics; **contract freeze declared in M6-T1 (issue #50)**; plugin-side pin = release-prep squash-merge `eda3bd9` (PR #117, nest-gated 2026-09-24 — `docs/agent-state/reports/2026-09-24-m6-pre-tag-smoke.md`); tag sits on the pin-fix commit carrying that exact pin (Variant A) |
