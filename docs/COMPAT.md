...
|------------|----------|--------|------------|
| On the **Lua keybind path**, release binds on a *modifier* key never fire | 2026-09-20 bisect on `efb5099`/`v0.56.2` host (`docs/agent-state/research/2026-09-20-lua-modifier-release-bisect.md`): `hl.bind("ALT + ALT_R"/"ALT + ALT_L", …, {release=true})` → no callback (modmask 8); ordinary keys with the same modmask fire (`F9` release modmask 0 — fires; `"ALT + TAB"` release modmask 8 — fires). Re-analysed 2026-09-21 at source level (`docs/agent-state/research/2026-09-21-modifier-hold-poll-api.md`, `KeybindManager.cpp:652,737-746`): the failure is **keybind current-mods matching**, a **keybind-path** limitation — not a statement that committing on Tab release equals apply-on-release | Apply-on-release silently broken for the documented `hl.bind("ALT_L", {release=true})` recipe on the Lua keybind path. The hyprlang `bindrt = ALT, ALT_L` path on the same pin was believed to keep working, but that claim **predates** the 2026-09-21 source analysis (same `KeybindManager` matching) and is **unverified** — Tab release is degraded UX, not a substitute (it commits a real focus move every Tab step) | **ADR-023:** Lua/Omarchy uses an explicit apply key — `ALT+Return` → `mru:apply` (`examples/mru-switcher-bindings.lua`, B1) — and/or a modifier-hold poll — `hl.is_key_down` (`Alt_L`/`Alt_R`) + `hl.timer` on the compositor main loop (`examples/mru-switcher-bindings-poll.lua`, B2). Tab-release is rejected as product semantics. hyprlang `bindrt = ALT, ALT_L` remains the intended route but its "keeps working" claim must be **empirically re-checked on the pin (nested run) before 1.0** (ADR-023) |
| On the **Lua config backend** (0.56.2 / `efb5099`), `plugin:mru-switcher:*` keys are **unsettable**: `hl.config` / the config-file parse reject them (`unknown config key 'plugin.mru-switcher.*'`), `hyprctl keyword` is disabled on non-legacy parsers | `docs/agent-state/reports/2026-09-22-lua-host-plugin-config-dead-end.md` (8 channels probed, all dead or unproven) | Plugin runs on compiled defaults (since ADR-025: `ui=border` → visual feedback out of the box; previously `ui=null`; M5 overlay unavailable) | **ADR-024:** opt-in sidecar file `~/.config/mru-switcher/config` (copy `examples/mru-switcher-sidecar.conf`) — same 14 SPEC §4 keys (incl. `pulse_period_ms` / `dim_alpha`, ADR-028), applied at load and on `hyprctl reload` (SPEC REQ-CFG-005). Upstream Lua `plugin {}` support is the long-term fix |
| On the **Lua config backend** (0.56.2 / `efb5099`), the **`dispatch` command is re-evaluated as Lua** (`hl.dispatch(<raw text>)`), so CLI/API `dispatch setprop …` with a bare `address:0x…` token is a Lua parse error and never reaches the compositor dispatcher (`hyprctl dispatch setprop` reproduces; 2026-09-22, host `efb5099`). `getprop` is unaffected | 2026-09-22 live Lua host: `hyprctl dispatch setprop address:0x…` → `')' expected near 'address'`; same root verified for plugin calls (`invokeHyprctlCommand("dispatch", …)`) in the 2026-09-22 border diagnosis | `ui=border` highlight silently never drawn on Lua hosts (every write failed; one-off toast) — the actual aspect of the reported "no border" bug | `HyprctlBorderPropIo::set` now calls `g_pKeybindManager->m_dispatchers["setprop"]` directly (legacy translator, bypasses the Lua shim, backend-agnostic). Verified 2026-09-22 nest: `mru:cycle next` writes `0xffffd9a0` to the selected window, session end restores the prior value |
| On the **Lua config backend** (0.56.2 / `efb5099`), `hyprctl dispatch <plugin:dispatcher> …` **cannot reach plugin dispatchers**: the Lua parser re-evaluates the dispatch text as `return hl.dispatch(<text>)`, so `mru:cycle next` is a Lua syntax error (`function arguments expected near 'next'`) — the same re-evaluation documented above for `dispatch setprop` | 2026-09-24 live host: `hyprctl dispatch mru:status` → `[string "return hl.dispatch(mru:status)"]:1: function arguments expected near ')'`; `hyprctl dispatch 'hl.plugin.mru.status()'` executes the bridge and only fails inside the wrapper (`hl.dispatch: expected a dispatcher`) — no nil-field error, so the bridge itself is registered and working | Scripted/manual smoke of the plugin through `hyprctl dispatch mru:*` is impossible on Lua hosts, so docs that suggested it were misleading | Drive the plugin from the Lua config (`hl.plugin.mru.cycle` / `apply` / `cancel` / `status`) — what `bindings.lua` already does (docs/USER.md "Dispatchers", docs/API.md); on hyprlang hosts `hyprctl dispatch mru:*` keeps working unchanged |
| **hyprpm does not autoload, and `hyprpm enable` needs root** (0.56.2, packaged `hyprpm`): `enable` / `add` / `update` stage state in `$XDG_RUNTIME_DIR/hyprpm/.temp-state` and install it as root (`NSys::root::install` → `sudo install -m644 -o 0 -g 0 …`; `SUPERUSER_BINARIES = {sudo, doas, run0}`), while hyprpm itself **refuses to run as root**; `hyprpm reload` only reads state and loads plugins over the Hyprland socket — no root, no state write | 2026-09-24 live Lua host `efb5099`: `hyprpm enable mru-switcher` → `✖ Failed to write plugin state` while `enabled = true` was already staged in `.temp-state`; journal `pam_unix(sudo:auth): conversation failed` (non-interactive sudo); `pkexec hyprpm …` → `Don't run hyprpm as a superuser`; source read: `hyprpm/src/helpers/Sys.cpp`, `hyprpm/src/core/DataState.cpp:30,37`, `hyprpm/src/main.cpp` (reload branch has no `cacheSudo`) | An "enabled" plugin is **absent after every reboot** until `hyprpm reload` runs, and the one-time `enable` cannot be automated from a non-interactive shell | Run `hyprpm enable <plugin>` once from a real terminal (hyprpm prompts for sudo itself). Add `hl.on("hyprland.start", function() hl.exec_cmd("hyprpm reload -n") end)` to the Lua autostart (hyprlang: `exec-once = hyprpm reload -n`) — README "Part 4", USER "Load the plugin on every login"; report `docs/agent-state/reports/2026-09-24-hyprpm-autoload-and-autostart.md` |
| On the **Lua config backend** (0.56.2 / `efb5099`) there is **no `hl.exec_once`**: the bundled stubs (`/usr/share/hypr/stubs/hl.meta.lua`) expose `hl.exec_cmd`, `hl.exec_raw` and `hl.on(event, cb)`, with `hyprland.start` among the event names | 2026-09-24 host inspection; Omarchy's own `/usr/share/omarchy/default/hypr/autostart.lua` and `hypr/helpers.lua:112` (`o.exec_on_start`) both use `hl.on("hyprland.start", …)` | A copy-pasted `hl.exec_once("…")` autostart line is a nil call at config load (visible in `hyprctl configerrors`) | Use `hl.on("hyprland.start", function() hl.exec_cmd("…") end)`; Omarchy shorthand `o.exec_on_start("…")`; hyprlang hosts: `exec-once = …` |
| On the **Lua config backend** (0.56.2 / `efb5099`), loading a **second** `mru-switcher` build (different path, same keys) while one is already loaded is **rejected**: `registerPluginValue` returns `name collision: already registered` → `addConfigValueV2` false → bare `failed to register config values`. A same-path re-load is rejected earlier by the host (`Cannot load a plugin twice!`). On hyprlang hosts a cross-path reload succeeds (verified nest 2026-09-22) | 2026-09-22 live host (Lua) + nest on `efb5099`/`v0.56.2` (hyprlang); host rejection chain traced at source (`src/config/lua/ConfigManager.cpp` `registerPluginValue`) | Operator confusion: `install.sh` + `hyprctl plugin load` after `hyprpm`/manual load failed with a misleading "crashed/threw in main" message | The plugin now names the colliding key and the unload-first hint in the error; operator flow: `hyprctl plugin list` → if loaded, `hyprctl plugin unload <path>` → `hyprctl plugin load <path>` |
...
| 1.0.0 | `efb50993780079460b0cbed1363e2166a2de1d9f` | `v0.56.2` | **nest gate PASS 2026-09-24** (`docs/agent-state/reports/2026-09-24-m6-pre-tag-smoke.md`); hyprpm install smoke pending tag (M6-T9); **stress smokes PASS 2026-09-19** (see §Stress evidence) | Single tested pin for 1.0 (Variant A): `hyprpm.toml` `commit_pins` = Hyprland `efb5099` → plugin `eda3bd9` (release-prep squash-merge, PR #117). Older untested multi-pin entries removed; repository metadata + build stanza verified by a clean-checkout build (M6-T5, issue #47) |
...
| Border highlight (M4, `ui=border`) | Per-window `setprop address:0x<ptr> active_border_color <color>` / `inactive_border_color <color>` invoked **in-process via `g_pKeybindManager->m_dispatchers["setprop"]`** (the compositor's legacy dispatcher translator, backend-agnostic; no focus side effect). **Not** via `HyprlandAPI::invokeHyprctlCommand("dispatch", …)` — on Lua-config hosts this pin re-evaluates `dispatch` as Lua (`hl.dispatch(<raw args>)`), so an unquoted `address:0x…` token is a parse error and `setprop` never runs (fixed 2026-09-22; Known host limitations row). Read path stays `invokeHyprctlCommand("getprop", …)` (backend-agnostic) | Unavailable/failed → `NullUI` + warn-once (REQ-UI-002); `IHyprWindowDecoration` documented fallback only
| Dim ring (M4, `ui=border`, `border_style=dim`; ADR-028, REQ-UI-014) | Same `setprop` path with **per-window `opacity` / `opacity_inactive`** (alpha). At the pin, `getprop … opacity` replies are plain finite decimals (`"1"`, `"0.7"`) which `setprop` accepts verbatim (normal format prints `valueOrDefault().alpha`, not the `overridden` flag). **Both** channels are overridden/restored because the render uses the active channel for the focused window and the inactive channel for every other window (`src/desktop/view/Window.cpp` `applyAlpha` on `alpha()`/`alphaInactive()`). Dim targets all valid non-selected ring windows; the selection keeps the highlight colour; prior alphas are restored by value. `dim_alpha >= 1.0` disables dimming | Unreadable alpha on a ring window → that window is skipped (fail-soft). **Fullscreen windows render via `alphaFullscreen()`, a third channel, so they may NOT appear dimmed** — documented limitation, no override of that channel (avoiding a per-window fullscreen-alpha game of whack-a-mole). Related: the ADR-028 design memo referred to the concept under the earlier name "dimAround"; the shipped behaviour is the per-window-alpha mechanism described here |
| Pulse throb (M4, `ui=border`, `border_style=pulse`; ADR-028, REQ-UI-013) | `wl_event_loop_add_timer(g_pCompositor->m_wlEventLoop, cb, this)` + `wl_event_source_timer_update` (one-shot; the callback re-arms) + `wl_event_source_remove` (`wayland-server-core.h`; same main-loop family as the overlay fd watch, ADR-019-style). Period = `pulse_period_ms/2` (half-cycle colour toggle: base ↔ `darker_color`, a pure re-scale of the RGB channels keeping alpha). Main-thread only; no compositor-touching background threads (AGENTS §7) | Timer source unavailable (no `g_pCompositor->m_wlEventLoop`, e.g. unit/null port) → constant colour + warn-once (fail-soft, REQ-UI-001); `schedule()` never throws out. A throwing port is caught and isolated like every other io |
| View-follow on selection (M4, `ui=border`; ADR-026, REQ-UI-012) | **Adapter-private** `HyprlandWorkspaceNavigator`: elevate the selected window's workspace on its own monitor with `CMonitor::changeWorkspace(const PHLWORKSPACE&, /*internal*/ false, /*noMouseMove*/ true, /*noFocus*/ true)` (or the `WORKSPACEID` overload) — **never** a focus move (no `focusWindow` call; REQ-UI-006/ADR-023 intact); restore each elevated monitor to its session-start workspace on cancel, leave views on apply. Only touches monitors whose active workspace is NOT the selection's target (already-visible selection = no-op). Backend-agnostic unit layer (`mru::plugin::WorkspaceNavigator`, gated by `selection_follow_workspace`, default `true`) delegates to it; `null`/degraded sessions use a no-op navigator | `CWorkspace::changeWorkspace` or any symbol that moves *focus* is **not** used. Unavailable/failed → warn-once, session continues with highlight only (REQ-UI-001) |
...
- **Primary mechanism (public props first, ADR-017 / REQ-UI-011):** per-window `setprop address:0x<ptr> active_border_color <color>` / `inactive_border_color <color>`. Since the 2026-09-22 Lua-host fix the write path calls the compositor's `setprop` dispatcher **directly** (`g_pKeybindManager->m_dispatchers["setprop"]`, legacy translator, in-process, no focus side effect) instead of `HyprlandAPI::invokeHyprctlCommand("dispatch", …)`: on Lua-config backends `dispatch` is re-evaluated as Lua and unquoted selector args fail to parse, so the hyprctl route could never colour a border there (getprop read route is backend-agnostic and unchanged). On hyprlang hosts both routes behaved identically. Wins above window rules/groups; window selector `address:0x<ptr>` (lowercase hex) matches the plugin's `WindowRef.address`.
