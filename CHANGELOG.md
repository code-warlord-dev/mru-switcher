...
- **Keybinding delivery is a first-class install step (ADR-022), fixing the Lua/Omarchy
  ...
  cycle/prev on `ALT + TAB` press, cancel on `ALT + Escape`, and commit the selection with an
  explicit apply key. The apply-on-**Tab**-release commit that first shipped in this bullet was a
  degraded host workaround, never the product model: it fires a real focus move on every Tab step,
  so the frozen list stops matching what the user sees and the browse-then-commit workflow is
  destroyed. Corrected later in the same Unreleased window — see the `### Fixed` entry on the apply
  model below and ADR-023. New opt-in `scripts/setup-bindings.sh`: autodetects the
...
- **`ui=border` highlight silently never drawn on Lua-config hosts (Omarchy, 0.56.2):** `HyprctlBorderPropIo::set` routed `setprop` through `HyprlandAPI::invokeHyprctlCommand("dispatch", …)`, but on Lua backends the `dispatch` command is re-evaluated as Lua (`hl.dispatch(<raw args>)`), so the unquoted `address:0x…` token is a parse error and every highlight write failed (one-off yellow toast, then silent). Root cause confirmed live 2026-09-22 (CLI `hyprctl dispatch setprop …` reproduces the same error) and at source (`HyprCtl` → Lua dispatch wrapper). Fix: call the compositor's `setprop` dispatcher in-process via `g_pKeybindManager->m_dispatchers["setprop"]` (runs the legacy dispatcher translator directly, bypassing the Lua shim; identical semantics on hyprlang and Lua hosts). Verified nested Hyprland on the fixed build: `mru:cycle next` writes `0xffffd9a0` to the selected window's `active_border_color`, and `mru:cancel`/session end restores the prior value. `getprop` unchanged (works on both backends). REQ-UI-003/004/005 intact; docs: USER.md FAQ + COMPAT.md Known host limitations row

- **Misleading `failed to register config values` on re-load (Lua hosts):** loading a second `mru-switcher` build while one is already loaded was rejected with a bare `crashed/threw in main: failed to register config values`, hiding the real cause — the host's `registerPluginValue` returns a **name collision** when the `plugin:mru-switcher:*` keys are already registered (Lua config backend 0.56.2; confirmed at source and on the live host 2026-09-22). `config::register_all` now returns the colliding key and `PLUGIN_INIT` surfaces it with an actionable hint ("if mru-switcher is already loaded, unload it first"), so the correct operator flow (`hyprctl plugin list` → `plugin unload <path>` → `plugin load <path>`) is discoverable. Same-path re-loads are rejected earlier by the host (`Cannot load a plugin twice!`); hyprlang hosts allow a cross-path re-load (nest-verified). No registration order or key change (REQ-CFG-002 intact); docs: USER.md FAQ + COMPAT.md Known host limitations row

- **Apply model corrected across docs and examples — explicit apply key for Lua, modifier-hold poll as the release reference, apply-on-Tab-release rejected (ADR-023):** the ADR-022 Lua recipe committed `mru:apply` on the release of **Tab**. That does fire on the pin (ordinary key, modmask 8 rebuild) but it is a degraded workaround, not a semantic equivalent of apply-on-release: every Tab step commits a real focus move, so the frozen MRU list stops matching what the user sees and the browse-then-commit workflow is destroyed. `examples/mru-switcher-bindings.lua` now cycles on `ALT + TAB` press and commits with an explicit `ALT + Return` apply key (nothing moves until it is pressed); the literal "release Alt to apply" feel ships as the new optional `examples/mru-switcher-bindings-poll.lua` — `hl.timer` on the compositor main loop (never a background thread) + `hl.is_key_down("Alt_L"`/`"Alt_R")`, armed between the first Tab press and the Alt release, disarmed before `apply()`, so apply runs exactly once and can never race `mru:cancel`. Load one variant or the other, never both. **Product messaging:** README's "Why release-on-Tab for Lua?" paragraph is replaced with an honest per-backend model — hyprlang keeps the classic `bindrt = ALT, ALT_L` release-Alt recipe, explicitly flagged as still needing an empirical nest re-verification on the pin (with an explicit apply key as the honest fallback), while Lua/Omarchy commits with `ALT + Return` and the poll variant is the documented route to literal Alt-release — plus the Omarchy `Alt+Tab` unbind requirement stated up front (the shipped Lua fragment unbinds; hyprlang users add `unbind = ALT, TAB`); `docs/USER.md` §3 and the `docs/API.md` Lua-bridge recipe drop the `{ release = true }` binds in favour of the explicit apply key and name the poll variant; `docs/COMPAT.md` mitigation text repointed from "use Tab release" to the explicit apply key / `hl.is_key_down` poll, and the hyprlang `bindrt` row honestly downgraded from "keeps working" to "re-verify before 1.0". Docs + examples only — no dispatcher, config-key, or plugin behavior change.
