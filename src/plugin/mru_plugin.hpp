...
// order — listeners -> controller -> ui -> overlay_socket -> border_io -> fg ->
// source -> tracker -> registry -> scheduler, then config/config_v2 — because
// implicit destruction (move-assignment) would run in declaration order and free
// the scheduler before the tracker's cancel_pending() dereferences it (REQ-H-008).
