...
// tracker must outlive it (ADR-015); the border UI holds a BorderPropIo&, so
// border_io must outlive it. The reverse order is enforced by an explicit
...
struct PluginState {
    ...
    std::unique_ptr<HyprctlBorderPropIo> border_io; // must outlive ui (ui holds BorderPropIo&)
    std::unique_ptr<mru::domain::UIPort> ui;        // NullUI | BorderHighlightUI (ADR-017)
