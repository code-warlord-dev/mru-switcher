...
SessionController::SessionController(WindowSource& source, FocusGateway& fg,
                                     UIPort& ui, HistoryTracker& tracker,
                                     SessionPolicy policy)
    : source_(source), fg_(fg), ui_(ui), tracker_(tracker),
      policy_(policy), snapshot_policy_(policy) {}

SessionController::CommandResult SessionController::cycle(Direction dir,
                                       std::optional<Scope> scope_override) {
  if (!active_)
...
SessionController::CommandResult SessionController::end_session(SessionEndReason reason,
